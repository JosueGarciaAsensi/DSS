...
<div class="container px-4">
    <div class="row">
        <div class="container col col-lg-2 mt-5 mb-5 p-4 rounded" style="background-color: black;">
      ...
      <div class="modal-body">
        <form action="{{ url('/admin-products/edit/' . $product->id)}}" method="POST">
            ...
            <div>
                ...
                <select name="beertype{{$product->id}}" id="beertype{{$product->id}}">
                @foreach($beertypes as $beertype)
                    @if(!is_null($beertype->id))
                        <option value="{{$beertype->id}}">{{$beertype->names}}</option>   
                    @endif
            ...
            <div class="form-group">
                ...
                <input type="number" id="stock{{$product->id}}" name="stock{{$product->id}}" class="form-control" placeholder="Stock" value="{{$product->stock}}" required>
            </div>
            <div class="form-group">
                <label for="price{{$product->id}}">Precio: </label>
                <input type="number" step="0.01" id="price{{$product->id}}" name="price{{$product->id}}" class="form-control" placeholder="Precio" value="{{$product->price}}" required>
            </div>
            <div class="form-group">
